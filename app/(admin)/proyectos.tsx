...
function diasRestantes(fechaLimite: string | null): { texto: string; color: string } | null {
  if (!fechaLimite) return null
  const diff = Math.ceil((new Date(fechaLimite).getTime() - Date.now()) / 86400000)
...
export default function Proyectos() {
  ...
  async function cargar() {
    ...
    const mapaPerfiles = new Map(perfilesData.map(p => [p.id, p.nombre]))
    const data = (projRes.data ?? []).map((p: any) => ({
      ...p,
      responsable_nombre: p.responsable_id ? (mapaPerfiles.get(p.responsable_id) ?? 'Desconocido') : null,
    })) as Proyecto[]
  ...
  async function cargarActividades(proyectoId: string) {
    setCargandoAct(true)
    ...
    if (editando) {
      ...
      if (user) {
        await supabase.from('proyecto_actividades').insert({
          proyecto_id: editando.id, user_id: user.id,
          descripcion: `Proyecto actualizado: estado → ${ESTADOS[form.estado]?.label ?? form.estado}`,
        })
      }
    } else {
      const { data: { user } } = await supabase.auth.getUser()
      const { data: nuevo, error } = await supabase.from('proyectos')
        .insert({ ...payload, creado_por: user?.id }).select().single()
      if (error) { alerta('Error: ' + error.message); setGuardando(false); return }
      if (nuevo && user) {
        await supabase.from('proyecto_actividades').insert({
          proyecto_id: nuevo.id, user_id: user.id, descripcion: 'Proyecto creado',
        })
      }
    }
  ...
  async function cerrarProyecto(p: Proyecto) {
    confirmar(`¿Marcar "${p.titulo}" como completado?`, async () => {
      ...
      if (user) {
        await supabase.from('proyecto_actividades').insert({
          proyecto_id: p.id, user_id: user.id, descripcion: 'Proyecto marcado como completado ✅',
        })
      }
      cargar()
    })
  ...
  async function agregarActividad() {
    ...
    const { data: { user } } = await supabase.auth.getUser()
    const { error } = await supabase.from('proyecto_actividades').insert({
      proyecto_id: detalle.id, user_id: user!.id, descripcion: nuevaAct.trim(),
    })
    setGuardandoAct(false)
    if (!error) { setNuevaAct(''); cargarActividades(detalle.id) }
  }
  ...
  const totalPorEstado = Object.keys(ESTADOS).reduce<Record<string, number>>((acc, e) => {
    acc[e] = proyectos.filter(p => p.estado === e).length
    return acc
  }, {})
  ...
  return (
    <View style={s.container}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.chipRow}>
        <TouchableOpacity
          style={[s.chip, !filtroPrioridad && s.chipActive]}
          onPress={() => setFiltroPrioridad(null)}
        >
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 60, gap: 12 }} showsVerticalScrollIndicator={false}>
          {filtrados.map(p => {
            ...
            return (
              <TouchableOpacity key={p.id} style={s.card} onPress={() => { setDetalle(p); cargarActividades(p.id) }} activeOpacity={0.85}>
              ...
              return (
                <>
                  <View style={s.detalleHeader}>
                    <View style={{ flex: 1 }}>
                      <Text style={s.detalleTitulo}>{detalle.titulo}</Text>
                  ...
                  <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, gap: 12 }} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={s.progressWrap}>
                      ...
                      <Text style={s.progressPct}>{detalle.progreso}%</Text>
                    </View>
                    ...
                    {/* Actividad */}
                    <Text style={s.actividadLabel}>Actividad</Text>
                    <View style={s.nuevaActRow}>
                      <TextInput
                        style={s.nuevaActInput}
                        value={nuevaAct}
                        onChangeText={setNuevaAct}
                        placeholder="Agregar nota de actividad..."
                    ...
                    {cargandoAct ? (
                      ...
                      <Text style={s.actVacia}>Sin actividad registrada aún.</Text>
                    ) : (
                      actividades.map(a => (
                  ...
                  <View style={s.detalleFooter}>
                    <TouchableOpacity style={s.btnEditarDetalle} onPress={() => { setDetalle(null); abrirEditar(detalle) }}>
                      ...
                      <Text style={s.btnEditarDetalleTxt}>Editar proyecto</Text>
                    </TouchableOpacity>
      ...
      <Modal visible={modal} animationType="slide" transparent onRequestClose={() => setModal(false)}>
        <View style={s.overlay}>
          <ScrollView style={s.formSheet} contentContainerStyle={{ paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
            ...
            <TextInput style={[s.input, { height: 80, textAlignVertical: 'top' }]} value={form.descripcion} onChangeText={v => setForm(f => ({ ...f, descripcion: v }))} placeholder="¿En qué consiste este proyecto?" multiline />
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
              <View style={{ flexDirection: 'row', gap: 6 }}>
                {Object.entries(ESTADOS).map(([key, cfg]) => (
                  <TouchableOpacity
                    key={key}
                    style={[s.chip, form.estado === key && { backgroundColor: cfg.color, borderColor: cfg.color }]}
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
              {Object.entries(PRIORIDADES).map(([key, cfg]) => (
                <TouchableOpacity
                  key={key}
                  style={[s.chip, { flex: 1, justifyContent: 'center' }, form.prioridad === key && { backgroundColor: cfg.color, borderColor: cfg.color }]}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              {[0, 25, 50, 75, 100].map(v => (
                <TouchableOpacity
                  key={v}
                  style={[s.chip, { flex: 1, justifyContent: 'center' }, form.progreso === v && s.chipActive]}
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
              <View style={{ flexDirection: 'row', gap: 6 }}>
                <TouchableOpacity
                  style={[s.chip, !form.responsable_id && s.chipActive]}
                ...
                {perfiles.map(p => (
                  <TouchableOpacity
                    key={p.id}
                    style={[s.chip, form.responsable_id === p.id && s.chipActive]}
...
const s = StyleSheet.create({
  ...
  kpiCard: {
    alignItems: 'center', padding: 10, borderRadius: 12, borderWidth: 1.5,
    borderColor: '#e2e8f0', backgroundColor: '#f8fafc', minWidth: 80, gap: 2,
  },
  kpiIcn: { fontSize: 18 },
  ...
  detalleSheet: { backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '90%', flex: 1 },
  ...
  actividadLabel: { fontSize: 13, fontWeight: '800', color: '#1a1a2e', marginTop: 4 },
  ...
  actVacia:     { fontSize: 12, color: '#94a3b8', fontStyle: 'italic' },
  ...
  btnCerrarDetalle:    { flex: 1, backgroundColor: '#16a34a', borderRadius: 12, paddingVertical: 12, alignItems: 'center' },
