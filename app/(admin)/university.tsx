...
export default function AdminUniversity() {
  ...
  async function cargar() {
    setLoading(true)
    const [{ data: statsData }, { data: cursosData }] = await Promise.all([
      supabase.rpc('get_vu_stats_admin'),
      supabase.from('vu_cursos')
        .select('id, titulo, nivel, publicado, categoria, vu_lecciones(id), vu_certificados(id)')
        .order('orden'),
    ])
  ...
  async function eliminarCurso(id: string, titulo: string) {
    const ok = await confirmar(`¿Eliminar "${titulo}"? Se borrará todo su contenido y progreso.`)
    if (!ok) return
        ...
        <>
          ...
          <View style={estilos.cursosHeader}>
            ...
            <TouchableOpacity
              ...
              onPress={() => router.push('/(admin)/university-curso-form')}
            >
          ...
          {cursos.length === 0 ? (
            <View style={estilos.empty}>
              ...
              <TouchableOpacity
                style={[estilos.btnNuevo, { marginTop: 16 }]}
                onPress={() => router.push('/(admin)/university-curso-form')}
              >
                ...
                <View style={estilos.cursoHeader}>
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={estilos.cursoMeta}>
                      ...
                      <Text style={estilos.cursoMetaText}>·</Text>
                ...
                <View style={estilos.cursoActions}>
                  <View style={estilos.publishRow}>
                    ...
                    <Switch
                      ...
                      onValueChange={() => togglePublicado(curso)}
                      trackColor={{ false: '#ddd', true: '#1a6470' }}
                      thumbColor={curso.publicado ? '#c9a84c' : '#fff'}
                    />
                  </View>
                  <View style={estilos.btnRow}>
                    <TouchableOpacity
                      style={estilos.btnEditar}
                      onPress={() => router.push(`/(admin)/university-curso-form?id=${curso.id}`)}
                    >
                      <Text style={estilos.btnEditarText}>✏️ Editar</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={estilos.btnEliminar}
                      onPress={() => eliminarCurso(curso.id, curso.titulo)}
                    >
