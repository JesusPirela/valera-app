...
// - Asesor (esAsesor=true): ve SOLO sus propias citas (asesor_id = él) y sus
//   propios seguimientos → su "dashboard de citas propio".
...
export default function CalendarioView({ esAsesor = false }: { esAsesor?: boolean }) {
  ...
  // Rutas al tocar una cita / seguimiento — dependen del rol.
  ...
  useEffect(() => {
    if (esAsesor && !miId) return   // el asesor filtra por su id: espera a tenerlo
    let vivo = true
    ...
    let qCitas = supabase.from('citas_coordinacion').select('id, fecha_cita, estado, clientes(nombre)')
      .not('fecha_cita', 'is', null).gte('fecha_cita', ini).lt('fecha_cita', fin)
    let qSegs = supabase.from('citas_venta').select('id, cliente_nombre, fecha_prox_seguimiento_ts')
    ...
    Promise.all([qCitas, qSegs]).then(([citas, segs]) => {
      ...
      const a: CalItem[] = (citas.data ?? []).map((x: any) => ({
        ...
        descripcion: x.estado ? String(x.estado).replace(/_/g, ' ') : null, color: '#2e7d32', tipo: 'cita', ruta: rutaCita,
      }))
  ...
  return (
    ...
    <View style={{ width: '100%', maxWidth: 760 }}>
      ...
      <Text style={[s.sub, { color: c.textMute }]}>Tus eventos + {esAsesor ? 'tus citas' : 'las citas del dashboard'} 🟢 y {esAsesor ? 'tus próximos seguimientos' : 'los próximos seguimientos'} 🟠.</Text>
