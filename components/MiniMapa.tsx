...
const SUBZONAS: Record<string, { label: string; coords: [number, number]; keywords: string[] }[]> = {
  queretaro: [
    ...
    { label: 'El Marqués',     coords: [20.6167, -100.2800], keywords: ['el marqués', 'el marques', 'marqués', 'marques'] },
    ...
    { label: 'Peñamiller',    coords: [21.0333, -99.8167], keywords: ['peñamiller', 'penamiller'] },
    { label: 'Centro',        coords: [20.5881, -100.3900], keywords: ['centro', 'centro histórico', 'centro historico'] },
  ],
  monterrey: [
    { label: 'San Pedro',      coords: [25.6500, -100.4000], keywords: ['san pedro', 'garza garcia', 'garza garcía'] },
...
export default function MiniMapa({ zonas, onZonaPress }: Props) {
  ...
  function pinHTML(count: number, color: string, size = 52) {
    const half = size / 2
    return `<div style="background:${color};color:#fff;border-radius:50%;width:${size}px;height:${size}px;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:${size === 52 ? 15 : 13}px;box-shadow:0 3px 12px rgba(0,0,0,0.35);border:3px solid #fff;cursor:pointer">${count}</div>`
  }
  ...
  function drillIntoCity(L: any, map: any, zona: ZonaPin) {
    ...
    // Match properties to sub-zones by keywords in direccion
    const props = zona.propiedades ?? []
    const subZonas = SUBZONAS[zona.key] ?? []
    const matched = new Set<number>()

    const subZonasConConteo = subZonas.map(sz => {
      const indices = props.reduce<number[]>((acc, p, i) => {
        if (sz.keywords.some(kw => p.direccion.toLowerCase().includes(kw))) { acc.push(i); matched.add(i) }
        return acc
      }, [])
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      const map = L.map(container, { center: [22.5, -102.55], zoom: 5, attributionControl: false })
      mapRef.current = map
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 18 }).addTo(map)
      renderCityPins(L, map, zonas)
    }
    ...
    if ((window as any).L) {
      ...
      const t = setInterval(() => { if ((window as any).L) { clearInterval(t); setTimeout(init, 50) } }, 100)
    }
