-- ══════════════════════════════════════════════════════════════════════════════
-- Fix: sincronizar_misiones_diarias_hoy no contaba seguimientos correctamente
--
-- El rework del 20260723 creó seguimientos_dia como fuente única de verdad:
-- cada vez que el prospectador trabaja un cliente (editar, completar recordatorio,
-- seguimiento rápido) se inserta UNA fila en seguimientos_dia por cliente por día.
-- registrar_seguimiento_cliente escribe ahí.
--
-- Pero sincronizar_misiones_diarias_hoy seguía leyendo recordatorios.completado_at,
-- que ya no es la fuente de verdad. Resultado: la misión "Al día con clientes"
-- no avanzaba aunque el prospectador diera seguimiento a sus clientes.
--
-- Fix: el bloque 'seguimiento' ahora cuenta desde seguimientos_dia.
-- ══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  LOOP
    ...
    ELSIF m.categoria = 'seguimiento' THEN
      -- Contar desde seguimientos_dia: fuente de verdad desde el rework del 20260723.
      -- Cada fila representa UN cliente trabajado hoy (deduplicado por cliente+día).
      SELECT COUNT(*) INTO v_count
