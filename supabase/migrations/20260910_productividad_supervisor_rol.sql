-- Agrega la columna `role` a la salida de get_productividad_equipo para poder
-- agrupar a los prospectadores por tipo (nuevo / prospectador / prospectador_plus)
-- en la nueva vista de estadísticas para supervisores.
--
-- IMPORTANTE: esta es una copia EXACTA de la versión vigente
-- (20260803_seguimientos_bloque_desde_seguimientos_dia.sql) con UNA SOLA línea
-- añadida (`p.role`). El supervisor YA estaba permitido en esa versión; NO se
-- toca ninguna otra parte del cuerpo (horas vía fn_conexion_diaria, seguimientos
-- vía seguimientos_dia, publicaciones vía publicacion_log, etc.).
--
-- [corrección] Una versión previa de esta migración copió por error el cuerpo
-- original de junio (20260609) y revirtió 5 meses de fixes — horas infladas por
-- sesiones abiertas, seguimientos en 0, etc. Este archivo restaura la versión
-- correcta y solo añade `role`.
  ...
  RETURN COALESCE((
    ...
    FROM (
      SELECT
        p.id,
        p.nombre,
        p.role,                        -- NUEVO: para agrupar por tipo de prospectador
