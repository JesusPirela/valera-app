-- ══════════════════════════════════════════════════════════════
-- FIX v2: Reescritura completa de ambas funciones de misiones diarias
-- Bug anterior: "IF um IS NULL" es siempre FALSE en PL/pgSQL.
-- La forma correcta es usar la variable especial FOUND después del SELECT INTO.
-- ══════════════════════════════════════════════════════════════

-- ── 1. incrementar_mision_diaria ─────────────────────────────
CREATE OR REPLACE FUNCTION public.incrementar_mision_diaria(
  p_categoria TEXT,
...
BEGIN
  FOR m IN
    ...
    WHERE tipo = 'diaria'
      ...
      AND activa = true
  LOOP
    -- Leer progreso actual con columnas explícitas (evita el problema de RECORD IS NULL)
    ...
    -- Si ya está completada hoy, no hacer nada
    ...
    IF NOT um_exists OR um_fecha IS NULL OR um_fecha < p_fecha THEN
      -- Día nuevo o sin registro: empezar en 1
      v_prog := 1;
    ELSE
      -- Mismo día: incrementar (sin bajar nunca)
      v_prog := LEAST(COALESCE(um_prog, 0) + 1, m.meta);
    END IF;
...
GRANT EXECUTE ON FUNCTION public.incrementar_mision_diaria(TEXT, DATE) TO authenticated;


-- ── 2. sincronizar_misiones_diarias_hoy ──────────────────────
...
BEGIN
  ...
  LOOP
    -- Contar actividad real de hoy en hora MX
    v_count := 0;
