-- ─────────────────────────────────────────────────────────────────────────────
-- Corrección de estadísticas de reportes de productividad
-- Problemas corregidos:
--   1. cursos_completados usaba vu_progreso (cualquier lección) → ahora usa
--      vu_certificados (cursos COMPLETAMENTE terminados, con certificado)
--   2. minutos_conexion usaba COALESCE(fin, NOW()) sin cap → sesiones sin fin
--      podían inflar el tiempo por días; ahora limita sesiones sin fin a 10 min
--      y cualquier sesión a 4 h máximo
--   3. ultimo_acceso ya no devuelve NOW() si la sesión está abierta
--   4. get_tendencia_equipo incluye citas_coordinacion y propiedad_actividad
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  RETURN COALESCE((
    ...
    FROM (
      ...
      -- CORREGIDO: contar certificados (cursos 100% completados) no lecciones sueltas
      LEFT JOIN LATERAL (
      ...
      -- CORREGIDO: sesiones sin fin → 10 min; cap por sesión 4 h; no usar NOW()
      LEFT JOIN LATERAL (
        SELECT
          COALESCE(SUM(
            LEAST(
          ...
          MAX(COALESCE(us.fin, us.inicio + INTERVAL '10 minutes')) AS ultimo_acceso
...
-- CORREGIDO: incluir citas_coordinacion y propiedad_actividad en la tendencia diaria
