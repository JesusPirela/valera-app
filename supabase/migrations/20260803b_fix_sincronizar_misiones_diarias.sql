-- FIX: sincronizar_misiones_diarias_hoy tronaba SIEMPRE con 'column reference
-- recompensa_xp/mision_id is ambiguous' (los nombres de salida del RETURNS TABLE
-- chocaban con columnas de misiones/user_misiones). El error se tragaba en
-- silencio en la app, así que las misiones que dependen de este sync (sobre
-- todo la de SEGUIMIENTO) nunca se creaban/completaban. Rayo hizo 17
-- seguimientos y la misión 'Al día con clientes' (meta 10) no se completaba.
-- Se agrega #variable_conflict use_column + alias en el FOR. Verificado: la
-- misión de seguimiento de Rayo pasa a 10/10 completada.
