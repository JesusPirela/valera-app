-- Rellena huecos de teléfono/prospecto/coordino/atendio en citas_venta cruzando
-- por nombre de cliente con el dashboard de citas (cualquier estado). Se llama al
-- terminar cada importación del Excel para que no se pierda el backfill.
...
BEGIN
  ...
  WITH citas AS (
    SELECT translate(lower(trim(cl.nombre)), 'áéíóúñ', 'aeioun') AS nn,
           cl.telefono, pc.nombre AS coord, pp.nombre AS prosp, pa.nombre AS ases, cc.asesor_id
