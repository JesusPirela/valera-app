-- Cuenta(s) que NO reciben leads de cofres (p. ej. cuenta de pruebas Alexis Plus).
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS no_leads_cofre boolean NOT NULL DEFAULT false;
UPDATE public.profiles SET no_leads_cofre = true WHERE id = '57176360-51bf-4256-add5-9353cf3752dd';
...
-- Cuentas marcadas con no_leads_cofre NUNCA reciben leads de cofres: se les
  -- dan monedas en su lugar (no consume el pool ni les llega el lead).
  IF p_tipo_premio IN ('lead_premium', 'lead_meta')
    ...
    VALUES (v_user_id, '🎁 Premio de cofre',
      'Te tocó un lead, pero tu cuenta no recibe leads de cofres. Te damos 200 Valera Coins en cambio.',
      'ruleta');
  ...
  -- diseno_pro: entregar un token de diseño profesional directamente.
  IF p_tipo_premio = 'diseno_pro' THEN
    ...
    VALUES (v_user_id, '🎨 ¡Ganaste un diseño profesional!',
      'Tienes un token de diseño. Úsalo desde cualquier propiedad con el botón "Solicitar diseño".',
      'ruleta');
  ...
  IF p_tipo_premio = 'patron_animado' THEN
    ...
    IF v_colores IS NOT NULL AND p_nombre_premio = ANY(v_colores) THEN
      ...
      RETURN jsonb_build_object(
        'ok', true, 'convertido', true, 'motivo', 'patron_duplicado',
        'mensaje', '🎨 ¡Ya tienes este patrón! Te damos 300 Valera Coins en cambio.'
      );
    END IF;

    UPDATE profiles
    SET colores_desbloqueados = array_append(COALESCE(colores_desbloqueados, ARRAY[]::TEXT[]), p_nombre_premio)
    WHERE id = v_user_id;
  ...
  IF p_tipo_premio = 'pack_color' THEN
    SELECT array_length(colores_desbloqueados, 1) INTO v_n_colores FROM profiles WHERE id = v_user_id;
    IF COALESCE(v_n_colores, 0) >= 12 THEN
      UPDATE user_stats SET valera_coins = valera_coins + 500 WHERE id = v_user_id;
      RETURN jsonb_build_object(
        'ok', true, 'convertido', true, 'motivo', 'coleccion_colores_completa',
  ...
  IF p_tipo_premio = 'pack_avatar' THEN
    SELECT array_length(avatares_desbloqueados, 1) INTO v_n_avatares FROM profiles WHERE id = v_user_id;
    IF COALESCE(v_n_avatares, 0) >= 16 THEN
      UPDATE user_stats SET valera_coins = valera_coins + 500 WHERE id = v_user_id;
      RETURN jsonb_build_object(
        'ok', true, 'convertido', true, 'motivo', 'coleccion_avatares_completa',
  ...
  SELECT p.id, '🎁 Premio ruleta pendiente',
    COALESCE((SELECT nombre FROM profiles WHERE id = v_user_id LIMIT 1), 'Un usuario') || ' ganó en la ruleta: ' || v_nombre_real,
    'ruleta'
  FROM profiles p WHERE p.role = 'admin';
